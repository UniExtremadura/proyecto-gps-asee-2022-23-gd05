...
public class CommandRecordList extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        AppExecutors.getInstance().diskIO().execute(new Runnable() {
            @Override
            public void run() {
                CommandRecord commandRecord = new CommandRecord("Test",3,"UserTest");
                AppDatabase.getInstance(getActivity()).getCommandRecordDao().insert(commandRecord);
                getActivity().runOnUiThread(()->mAdapter.add(commandRecord));
            }
        });
